...
void worker(Queue& q, int tid, int num_ops, Workload workload, std::barrier<>& sync, int num_threads)
{
    ...
    double ratio = 0.0;
    switch (workload) {
        case Workload::EnqueueHeavy:       ratio = 0.67; break;
        case Workload::DequeueHeavy:       ratio = 0.33; break;
        case Workload::EnqueueDequeuePair: ratio = 1.00; break;
    }
    ...
    if (workload == Workload::EnqueueDequeuePair) {
        ...
        for (int op : ops) {
            if (op == 0) q.enqueue(1, tid);
            else         q.dequeue(&value, tid);
        }
    }
